...
class API
{
    ...
    public function checkTable($query_table, $db = null)
    {
        ...
        $tables = $this->getCache($this->getDatabase($db)->name . '_tables');
    ...
    public function checkColumn($column, $table, $db = null)
    {
        ...
        if (!$this->auth->is_admin || isset($_SERVER['HTTP_REFERER']) || Request::method() == 'PUT') {
    ...
    public function getColumns($table, $db = null)
    {
        ...
        $key = $this->getDatabase($db)->name . '.' . $table . '_columns';
